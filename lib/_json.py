...
on psycounvdb.extension, so I can't create the default JSON typecasters in
...
class Json:
    """
    An `~psycounvdb.extensions.ISQLQuote` wrapper to adapt a Python object to
    :sql:`json` data type.
...
def register_json(conn_or_curs=None, globally=False, loads=None,
    ...
    are provided.  Raise `~psycounvdb.ProgrammingError` if the type is not found.
